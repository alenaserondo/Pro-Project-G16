...
void setup()
{
  ...
  //status dropdown - alyx harmon 25/3 8.50pm
  ...
  //Screens - Alyx Harmon
  // updated 16/03/2026
  // updated and new filter by date screen added 26/3 9.50am
...
void draw()
{
  ...
  if (currentScreen == screens.get(2))
  {
    ...
    int maxStart = filteredFlights.size() - visibleFlights;
    ...
    for (int i = 0; i < visibleFlights; i++)
    {
      ...
      if (index < filteredFlights.size())
      {
        Flights flight = filteredFlights.get(index);
