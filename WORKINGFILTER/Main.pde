...
void customizeAirlineDDL() { 
 ...
 airlineDDL.setColorBackground(color(183, 144, 212));   // main bar
  ...
  airlineDDL.setColorActive(color(140, 100, 190));       // clicked
...
void customizeFlightDDL() { 
  ...
  flightDDL.setColorBackground(color(183, 144, 212));   // same base
...
//Alena 26.03.26 = filter for flightnum and airlinecode
...
void draw()
{
  ...
  if (enteredText.length() > 0) {
      listToShow = airportFilter;  // airport text filter
  } else if (filtered.size() > 0) {
      listToShow = filtered;       // airline + flight filter
  } else {
      listToShow = flights;        // default
  }
